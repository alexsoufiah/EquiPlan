...
export async function POST(req: NextRequest) {
  ...
  const { shift_id, helper_id, notes, role } = await req.json();
  ...
  const result = db.prepare(
    "INSERT INTO shift_assignments (shift_id, helper_id, worker_name, notes, role) VALUES (?, ?, ?, ?, ?)"
  ).run(shift_id, helper_id, worker_name, notes ?? null, role ?? null);
...
export async function PUT(req: NextRequest) {
  ...
  const { id } = body;
  const db = getDb();
  // tournament_id + aktuelle Werte ermitteln
  const cur = db.prepare(
    "SELECT s.tournament_id, sa.attended, sa.notes, sa.role FROM shift_assignments sa JOIN shifts s ON sa.shift_id = s.id WHERE sa.id = ?"
  ).get(id) as { tournament_id: number | null; attended: number; notes: string | null; role: string | null } | undefined;
  if (!cur) return NextResponse.json({ error: "Zuweisung nicht gefunden" }, { status: 404 });
  if (!canManageTournament(session, cur.tournament_id)) return NextResponse.json({ error: "Kein Zugriff auf dieses Turnier" }, { status: 403 });

  // Nur mitgeschickte Felder ändern (Anwesenheit / Notiz / Rolle); der Helfer bleibt fix
  const attended = body.attended !== undefined ? body.attended : cur.attended;
  const notes = body.notes !== undefined ? body.notes : cur.notes;
  const role = body.role !== undefined ? body.role : cur.role;
  db.prepare("UPDATE shift_assignments SET attended=?, notes=?, role=? WHERE id=?")
    .run(attended ?? 0, notes ?? null, role ?? null, id);
